...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.config.onDidChange("claude-chat.pulsarMCP", ({ newValue }) => {
        if (newValue) {
          this.startMcpBridge();
        } else {
          this.stopMcpBridge();
  ...
  startMcpBridge() {
    try {
      ...
      log.debug("Starting MCP bridge", { basePort, port });

      this.mcpBridge = startBridge({ port });
      this.mcpBridgePort = port;
      registerPort(port);

      log.debug(`MCP bridge started on port ${port}`);
    } catch (error) {
  ...
  stopMcpBridge() {
    if (this.mcpBridge) {
      ...
      stopBridge(this.mcpBridge)
        .then(() => log.info("MCP bridge stopped"))
