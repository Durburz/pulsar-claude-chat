...
export default {
  ...
  async toggle() {
    ...
    // Use panel's actual URI if it exists (changes after session starts)
    const uri = this.panel?.getURI() || `${ChatPanel.URI_PREFIX}/panel`;
    const item = await atom.workspace.toggle(uri, { location });
    // Only focus if panel was shown (toggle returns the item when shown, undefined when hidden)
    if (item) {
  ...
  // Helper to open panel and set context
  ...
  provideService() {
    return {
       ...
       * @param {Object} options.attachContext - Attach context (selection, paths, etc.)
       * @param {boolean} options.focus - Whether to focus the panel after sending (default: true)
      ...
      sendPrompt: async (text, options = {}) => {
        ...
        // Focus panel unless explicitly disabled
       ...
       * @param {Function} callback - Called with message object { role, content, thinking? }
       * @returns {Disposable} - Dispose to unsubscribe
       */
