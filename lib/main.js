...
import { startBridge, stopBridge, setExternalTools } from "./pulsar-mcp/bridge";
...
// External tool renderer configs registered by other packages
const externalRendererConfigs = new Map();

/**
 * Get external renderer configs for tool-renderers.js
 * @returns {Map} Map of tool name to renderer config
 */
export function getExternalRendererConfigs() {
  return externalRendererConfigs;
}
...
export default {
   ...
   *
   * @param {Object} provider - Tool provider object
   * @param {Function} provider.getTools - Returns array of tool definitions
   * @returns {Disposable} - Disposable to unregister tools when package deactivates
   */
  consumeMcpTools(provider) {
    if (!provider || typeof provider.getTools !== "function") {
      log.error("Invalid MCP tools provider: must have getTools() method");
      return new Disposable();
    }

    const tools = provider.getTools();
    if (!Array.isArray(tools)) {
      log.error("Invalid MCP tools provider: getTools() must return an array");
      return new Disposable();
    }

    const registeredNames = [];
    for (const tool of tools) {
      if (!tool.name || !tool.impl) {
        log.error("Invalid tool definition: must have name and impl", { tool });
        continue;
      }
      ...
      // Register renderer config (custom or default empty for pulsar-mcp-tool class)
      const mcpToolName = `mcp__pulsar__${tool.name}`;
      externalRendererConfigs.set(mcpToolName, tool.renderer || {});
      log.debug(`Registered external renderer config: ${mcpToolName}`);
    }
    ...
    return new Disposable(() => {
      for (const name of registeredNames) {
        ...
        externalRendererConfigs.delete(`mcp__pulsar__${name}`);
      }
      setExternalTools(externalTools);
