...
import { getExternalRendererConfigs } from "./main";
...
export const renderToolAgentOutput = createSimpleToolRenderer("AgentOutput", (input) => ({
  code: input?.agentId,
...
const TOOL_RENDERERS = {
  ...
  AgentOutputTool: renderToolAgentOutput,
...
/**
 * Content type renderers for external tools
 * Maps contentType to a render function
 */
const CONTENT_TYPE_RENDERERS = {
  "linter-messages": (msg, handlers) => {
    const data = parseJsonResult(msg.result);
    ...
    return (
      <div className="tool-linter-list">
        {data.slice(0, 50).map((item, i) => (
          <div className={`linter-item linter-${item.severity || "info"}`} key={i}>
            <span className={`linter-icon icon icon-${severityIcon(item.severity)}`} />
            ...
            {item.file ? (
              <a
                ...
                on={{
                  click: (e) => {
                    e.preventDefault();
                    handlers.openFile(item.file, item.range?.start?.row);
                  },
                }}
              >
                {item.file.split(/[/\\]/).pop()}
                {item.range?.start?.row != null ? `:${item.range.start.row + 1}` : ""}
              </a>
            ) : null}
            {item.linterName ? <span className="linter-name">{item.linterName}</span> : null}
          </div>
        ))}
        {data.length > 50 ? <div className="linter-more">... {data.length - 50} more</div> : null}
      </div>
    );
  },
};
...
/**
 * Build a renderer from external config
 * @param {Object} config - Renderer configuration from external package
 * @returns {Function} Renderer function
 */
function buildExternalRenderer(config) {
  const contentRenderer = CONTENT_TYPE_RENDERERS[config.contentType];

  return createToolRenderer({
    name: config.name || "Tool",
    className: config.className || "mcp-external",
    extraClasses: ["tool-mcp-pulsar"], // Inherit "Pulsar/" prefix styling
    getInfo: (input, result) => {
      const data = parseJsonResult(result);
      return config.getInfo ? config.getInfo(input, data) : {};
    },
    hasExpandable: (input, result) => {
      const data = parseJsonResult(result);
      return config.hasExpandable ? config.hasExpandable(input, data) : false;
    },
    renderContent: contentRenderer
      ? (msg, handlers) => contentRenderer(msg, handlers)
      : (msg) => renderPreContent(JSON.stringify(parseJsonResult(msg.result), null, 2), 1000),
  });
}

/**
 * Cache for built external renderers
 */
const externalRendererCache = new Map();

/**
 * Get or build an external renderer
 * @param {string} toolName - MCP tool name (e.g., "mcp__pulsar__GetLinterMessages")
 * @returns {Function|null} Renderer function or null
 */
function getExternalRenderer(toolName) {
  // Check cache first
  if (externalRendererCache.has(toolName)) {
    return externalRendererCache.get(toolName);
  }

  // Check for config
  const configs = getExternalRendererConfigs();
  ...
  const renderer = buildExternalRenderer(config);
  externalRendererCache.set(toolName, renderer);
  return renderer;
}

export function renderTool(msg, index, handlers) {
  // Check builtin renderers first
  const builtinRenderer = TOOL_RENDERERS[msg.name];
  if (builtinRenderer) {
    return builtinRenderer(msg, index, handlers);
  }

  // Check external renderers
  const externalRenderer = getExternalRenderer(msg.name);
  if (externalRenderer) {
    return externalRenderer(msg, index, handlers);
  }
