  ...
  return (
    ...
    >
      <div className="tool-header">
        ...
        >
          {isError ? <span className="icon-alert"></span> : <span className="icon-terminal"></span>}
          Bash
        </span>
        {description ? (
...
export function renderToolSearch(msg, index, handlers) {
  const { id, name, input, result, collapsed } = msg;
  ...
  return (
    <div
      className={`message message-tool tool-search ${collapsed ? "collapsed" : ""}`}
      key={index}
    >
      <div className="tool-header">
        <span
