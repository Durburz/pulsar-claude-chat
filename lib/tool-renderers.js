  ...
  if (Array.isArray(result)) {
    for (const block of result) {
      if (block.type === "image") {
        ...
        if (data) return { type: "image", mediaType, data };
      }
    }
  }
  return null;
...
function parseSearchResults(result, maxEntries = 50) {
  ...
  return result.trim().split("\n").filter((l) => l).slice(0, maxEntries).map((line) => {
    const match = line.match(/^(.+?):(\d+):(.*)$/);
    if (match) return { path: match[1], line: parseInt(match[2]), content: match[3] };
    return { path: line };
  });
}
...
export const renderToolRead = createToolRenderer({
  ...
  getInfo: (input, result) => ({
    ...
    pathInfo: getReadLineRange(input),
    resultInfo: parseReadResult(result),
  }),
  ...
  renderContent: (msg) => {
    const preview = getReadPreview(msg.result);
      ...
      <div className="tool-read-content">
        ...
        {hasText ? (
          <pre className="tool-content">
            ...
            {msg.result.length > 5000 ? `\n... (${msg.result.length - 5000} more chars)` : ""}
          </pre>
        ) : null}
      </div>
    );
  ...
  return (
    ...
    >
      <div className="tool-header">
        ...
        {searchPath ? <span className="tool-path-info">{searchPath}</span> : null}
...
export function renderTool(msg, index, handlers) {
  ...
  return renderer ? renderer(msg, index, handlers) : renderToolDefault(msg, index);
}
