...
import { listSessions, deleteSession, clearAllSessions, formatSessionForList } from "./session-store";
  ...
  constructor(main) {
    ...
    super({
      ...
      didConfirmSelection: async (item) => {
        self.instance.hide();
        await main.openSession(item.sessionId);
      },
      ...
      listCommands: {
        "claude-chat:open-keep-list": async () => {
          ...
          if (item) await main.openSession(item.sessionId);
        },
