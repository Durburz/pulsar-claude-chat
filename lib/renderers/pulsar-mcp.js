...
export const renderMcpSetTextInRange = createToolRenderer({
  name: "SetTextInRange",
  className: "mcp-pulsar",
  getInfo: (input, result) => {
    const data = parseMcpResult(result);
    ...
    const end = input?.end;
    return {
      resultInfo: start && end ? `[${start.row + 1}:${start.column + 1}]-[${end.row + 1}:${end.column + 1}]` : null,
      badge: data?.replaced === false ? "failed" : null,
    };
  },
  hasExpandable: (input) => input?.text != null,
  renderContent: (msg) => {
    const text = msg.input?.text;
    if (text == null) return null;
    if (text.length === 0) {
      return <div className="tool-mcp-content"><em className="mcp-empty-text">(empty - delete)</em></div>;
    }
    return renderPreContent(text, 500);
  },
});
...
export const renderMcpGetLinterMessages = createToolRenderer({
  ...
  getInfo: (input, result) => {
    ...
    return {
      ...
      countLabel: "issues",
      resultInfo: parts.length > 0 ? parts.join(" ") : null,
    };
  },
  hasExpandable: (input, result) => {
    const data = parseMcpResult(result);
...
export const MCP_TOOL_RENDERERS = {
  ...
  "mcp__pulsar__SetTextInRange": renderMcpSetTextInRange,
