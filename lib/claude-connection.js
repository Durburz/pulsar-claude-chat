...
export default class ClaudeConnection {
  ...
  start(options = {}) {
    ...
    // Add MCP config for Pulsar integration (if enabled)
    if (atom.config.get("claude-chat.pulsarMCP")) {
      // Get actual port from main module (handles multi-window port allocation)
      const mainModule = atom.packages.getActivePackage("claude-chat")?.mainModule;
      const mcpPort = mainModule?.getMcpBridgePort() || atom.config.get("claude-chat.mcpBridgePort") || 3000;
      const serverPath = path.join(__dirname, "pulsar-mcp", "server.js");
      const mcpConfig = {
        mcpServers: {
          pulsar: {
            command: "node",
