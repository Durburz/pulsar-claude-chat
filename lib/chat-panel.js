...
export default class ChatPanel {
  ...
  constructor(props = {}) {
    ...
    // Thinking state - use config helper
    this.thinkingMode = props.thinkingMode ?? Config.thinkingMode();
   ...
   * Used when changing thinking mode or permission mode
   */
  recreateConnection() {
  ...
  formatAttachContext() {
    ...
    if (type === "selection" && selection) {
      // Code selection with file location
      return `User is referring to this code from ${paths[0]} (line ${line}):\n\`\`\`\n${selection}\n\`\`\`\n\n`;
    } else if (type === "position") {
      // Cursor position reference
      return `User's cursor is at ${paths[0]}:${line}:${column}\n\n`;
    } else if (type === "image") {
      ...
      if (paths.length === 1) {
        return `User is referring to: ${paths[0]}\n\n`;
      }
          ...
          <div className="claude-chat-toolbar">
            ...
            <div className="toolbar-actions">
              ...
              <div className="btn-group send-group">
                <button
                  ref="thinkingBtn"
                  className={`btn icon icon-light-bulb ${this.thinkingMode ? "selected" : ""}`}
