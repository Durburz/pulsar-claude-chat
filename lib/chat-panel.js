...
export default class ChatPanel {
      ...
      this.connection.on("tool-use", ({ id, name, input }) => {
        ...
        const collapsed =
          this.defaultToolCollapsed !== null
            ...
            : name !== "TodoWrite"; // TodoWrite starts expanded by default
        this.messages.push({
          role: "tool",
          id,
          name,
  ...
  /**
   * Send a prompt programmatically (for use by service API)
   * @param {string} text - The prompt text to send
   * @param {Object} attachContext - Optional attach context
  ...
  sendPrompt(text, attachContext = null) {
    ...
    // Set attach context if provided
    if (attachContext) {
